defmodule QuickAuction.FrontendWeb.Components.Auction do
  ...
  attr(:remaining_time, Time, required: true)
  ...
  def info(assigns) do
    ...
    <div class="px-6 pt-4 pb-2 w-full">
      ...
      <div class="flex flex-row text-l mb-2">
        <div class="font-bold">Time remaining</div>
        <div class="flex-grow text-right"><%= time_until(@remaining_time) %></div>
      </div>
      <div class="flex flex-row text-l mb-2">
  ...
  defp time_until(timestamp) do
    timestamp |> Time.to_iso8601()
  end
